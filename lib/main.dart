...
import 'package:firebase_flutter/movies.dart';
...
class _FilmListState extends State<FilmList> {
  ...
  Widget build(BuildContext context) {
    ...
    switch (_filterOrSort) {
      ...
      case "sort_popularity":
        ...
        /// Order by the popularity, and return only those which has one great than 90
        query = query.orderBy('popularity').where('popularity', isGreaterThan: 50);
        break;
      case "filter_genre_thriller":

        /// Return the movies which have the following categories
        query = query.where('genre', arrayContainsAny: ['Thriller']);
        break;
      case "filter_genre_action":

        /// Return the movies which have the following categories
        query = query.where('genre', arrayContainsAny: ['Action']);
        break;
    }
          ...
          PopupMenuButton(
            ...
            itemBuilder: (BuildContext context) {
              return [
                ...
                PopupMenuItem(
                  value: "filter_genre_action",
                  child: Text("Filter genre Action"),
                ),
                PopupMenuItem(
                  value: "filter_genre_thriller",
                  child: Text("Filter genre Thriller"),
                ),
                PopupMenuItem(
  ...
  addMovies() async {
    ...
    movies.forEach((element) {
      print(
          "무비 엘리먼트 [${element.title}-${element.year}-${element.rated}-${element.popularity}-${element.likes}]");
...
class Movie extends StatelessWidget {
  ...
  Widget get title {
    return Text("${movie['title']} (${movie['year'].substring(0, 4)})",
  ...
  List<Widget> genreItems() {
    ...
    movie['genre'].forEach((genre) {
      items.add(Padding(
        child: Chip(
            label:
                Text(genre.toString(), style: TextStyle(color: Colors.white)),
